...
export default function App() {
  ...
  const handleAddProduct = async (e) => {
    e.preventDefault();
    const fd = new FormData(e.target);
    const comboPriceVal = parseFloat(fd.get('comboPrice'));
    ...
    const newP = {
      ...
      costPrice: parseFloat(fd.get('costPrice')),
      sellPrice: parseFloat(fd.get('sellPrice')),
      comboPrice: isNaN(comboPriceVal) ? null : comboPriceVal,
      stock: parseInt(fd.get('stock'), 10),
      minStock: parseInt(fd.get('minStock'), 10),
    };
  ...
  const handleUpdateProduct = async (e) => {
    ...
    const updatedData = {
      ...
      costPrice: isNaN(newCost) ? editingProduct.costPrice : newCost,
      sellPrice: isNaN(newSell) ? editingProduct.sellPrice : newSell,
      comboPrice: isNaN(newCombo) ? null : newCombo,
      minStock: parseInt(fd.get('minStock'), 10) || editingProduct.minStock
    };
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 w-full max-w-md rounded-3xl p-6 space-y-4 shadow-2xl">
            ...
            <form onSubmit={handleAddProduct} className="space-y-3 text-xs">
              <div>
                <label className="text-slate-400 block mb-1">Nombre del Producto:</label>
                <input required name="name" placeholder="Ej: Bolsa de Hielo 4kg / Vaso Plástico" className="w-full bg-slate-950 border border-slate-800 rounded-xl p-2.5 text-white" />
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="text-slate-400 block mb-1">Marca:</label>
                  <input required name="brand" placeholder="Benga" className="w-full bg-slate-950 border border-slate-800 rounded-xl p-2.5 text-white" />
                </div>
                <div>
                  <label className="text-slate-400 block mb-1">Categoría:</label>
                  <input required name="category" placeholder="Hielo / Vasos / Insumos" className="w-full bg-slate-950 border border-slate-800 rounded-xl p-2.5 text-white" />
                </div>
              </div>
              <div className="grid grid-cols-3 gap-2">
                <div>
                  <label className="text-slate-400 block mb-1">Precio Costo ($):</label>
                  <input required type="number" step="any" name="costPrice" placeholder="500" className="w-full bg-slate-950 border border-slate-800 rounded-xl p-2.5 text-white font-mono" />
                </div>
                <div>
                  <label className="text-slate-400 block mb-1">Precio Venta ($):</label>
                  <input required type="number" step="any" name="sellPrice" placeholder="1200" className="w-full bg-slate-950 border border-slate-800 rounded-xl p-2.5 text-white font-mono" />
                </div>
                <div>
                  <label className="text-cyan-400 font-bold block mb-1">P. Combo ($):</label>
                  <input type="number" step="any" name="comboPrice" placeholder="0 (Regalo) o 500" className="w-full bg-slate-950 border border-cyan-500/40 rounded-xl p-2.5 text-white font-mono focus:border-cyan-400 focus:outline-none" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="text-slate-400 block mb-1">Stock Inicial:</label>
                  <input required type="number" name="stock" placeholder="20" className="w-full bg-slate-950 border border-slate-800 rounded-xl p-2.5 text-white font-mono" />
                </div>
                <div>
                  <label className="text-slate-400 block mb-1">Stock Mínimo Alerta:</label>
                  <input required type="number" name="minStock" placeholder="5" className="w-full bg-slate-950 border border-slate-800 rounded-xl p-2.5 text-white font-mono" />
            ...
            <form onSubmit={handleUpdateProduct} className="space-y-3 text-xs">
              ...
              <div className="grid grid-cols-3 gap-2">
                <div>
                  ...
                  <input required type="number" step="any" name="costPrice" defaultValue={editingProduct.costPrice} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-2.5 text-white font-mono" />
                </div>
                <div>
                  <label className="text-fuchsia-400 font-bold block mb-1">Precio Venta ($):</label>
                  <input required type="number" step="any" name="sellPrice" defaultValue={editingProduct.sellPrice} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-2.5 text-white font-mono" />
                </div>
                <div>
                  <label className="text-cyan-400 font-bold block mb-1">P. Combo ($):</label>
                  <input type="number" step="any" name="comboPrice" defaultValue={editingProduct.comboPrice ?? ''} placeholder="Ej: 0 o 500" className="w-full bg-slate-950 border border-cyan-500/50 rounded-xl p-2.5 text-white font-mono focus:border-cyan-400 focus:outline-none" />
                </div>
              </div>
              <div>
                <label className="text-slate-400 block mb-1">Stock Mínimo Alerta:</label>
